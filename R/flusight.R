...
export_flusight_csv <- function(x, dir=NULL, overwrite=F) {
  ...
  x <- as.predx_df(x) %>%
    ...
    dplyr::mutate(
      ...
      bin_start_incl = ifelse(unit == 'percent' & type == 'Bin', sprintf("%.1f", lwr), NA),
      bin_start_incl = ifelse(unit == 'week' & type == 'Bin', ifelse(cat == "none", "none", paste0(cat, ".0")), bin_start_incl),
      bin_end_notincl = ifelse(unit == 'percent' & type == 'Bin', sprintf("%.1f", lwr + 0.1), NA),
      bin_end_notincl = ifelse(unit == 'percent' & type == 'Bin' & lwr == 13, sprintf("%.1f", 100), bin_end_notincl),
